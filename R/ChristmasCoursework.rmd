...
Using the likelihood ratio test (which is $\chi^2 (4)$ distributed) we fail to reject the null hypothesis that the restrictions are not binding, or more simply we can not say the restrictions are binding.
...
qchisq(0.025, df=1, lower.tail = F)
```

Using AIC we marginally choose the ARMA model over the random walk.
