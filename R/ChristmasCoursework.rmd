...
Similarly another BPG test using the lmtest package returns a p-value < 0.05 so we reject the null hypothesis of homoskedasticity. We can say our estimator is unbiased but not minimum variance and efficient. 
...
Performing a RESET test to inform us on the functional form we see a RESET test statistic of 7.13 and a p-value of 0.001. We reject the null hyphothesis of non-linearity or correct functional form
```{r echo=FALSE}
...
```{r echo=FALSE}
knitr::kable(data.frame(AIC(ur.ardl.model), AIC(r.model), BIC(ur.ardl.model), BIC(r.model)), caption = "AIC & BIC comparisons for unrestricted (ur) and restricted (r) models")
```
...
print(paste("Chi Sq CV:", round(qchisq(p=0.05, df=4, lower.tail = F), 2)))
```
...
We test a 4 variable VAR with $INF_t$, $RL_t$, $U_t$ and $G_t$. First we look at possible Granger Causality between the variables. We do not include all the GC tests in our results but we include those with the greatest significance. We can say reject the null hypothesis of no Granger Causality between 

* inflation and unemployment, $INF_t$ and $U_t$. 
