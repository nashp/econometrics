...
#rep(1, nrow(macro.series))
X = na.omit(cbind(rep(1, nrow(macro.series)),macro.series[, "U"], stats::lag(macro.series[, c("INF")])))
colnames(X) <- c("U", "INF")
W = na.omit(cbind(rep(1, nrow(macro.series)), stats::lag(macro.series[, c("INF", "U", "G", "RL")])))
colnames(W) <- c("INF", "U", "G", "RL")
s1.data <- cbind(X,W)
s1.model <- dyn$lm(U ~ INF + U.1 + G + RL, data=s1.data)
